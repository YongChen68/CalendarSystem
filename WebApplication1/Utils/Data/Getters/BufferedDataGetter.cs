...
namespace CalendarSystem.Utils.Data
{
    public class BufferedDataGetter : IGetter
    {
        ...
        public List<InstallationEvent> GetData()
        {
            ...
            return Lift.LiftManager.DbHelper.ReadObjects<InstallationEvent>(SQL);
        }
        ...
        //where d.ScheduledDate >= '{0} 12:00' and d.ScheduledDate <= '{0} 11:59'
        ...
        //'Pending Install Completion') and Branch in ('{1}') and i.Recordid in (select ParentRecordId from #dates group by ParentRecordId)

        //insert into #installs select i.* from HomeInstallations i
        //where CurrentStateName in ('Unreviewed Buffered Work', 'Buffered Work') and PlannedInstallWeek = {2} and RecordId not in (select ParentRecordId from #dates d group by ParentRecordId) and Branch in ('{1}')
